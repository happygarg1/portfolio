    ...
    >
      <div className={`${id === 6 && 'flex justify-center'} h-full`}>
        <div className="w-full h-full absolute">
          {img && (
            <Image src={img} alt={img} width={100} height={100} className={cn(imgClassName, 'object-cover,object-center')}
            />
          )}
        </div>
        <div className={`absolute right-0 -bottom-5 ${id === 5 && 'w-full opacity-80'}`}>
          {spareImg && (
            <Image 
              src={spareImg}
              alt={spareImg} width={100} height={100}
          ...
          <div className="flex gap-1 lg:gap-4 w-fit absolute -right-3 lg:-right-2">
            ...
            <span className="py-4 px-3 rounded-lg text-center bg-[#10132E]"></span>
              {['React.js','Next.js','TypeScript'].map((item)=>(
                <span key={item} className="lg:py-3 lg:px-2 py-2 px-3 text-xs lg:text-[16px] opacity-50 
            ...
            <span className="lg:mt-4 py-4 px-3 rounded-lg text-center bg-[#10132E]"></span>
              {['node.js','EJS','MongoDb'].map((item)=>(
                <span key={item} className=" lg:py-4 lg:px-3 py-2 px-3 text-xs lg:text-[16px] opacity-50 
                    lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                      {item}
                    </span>
              ))}
